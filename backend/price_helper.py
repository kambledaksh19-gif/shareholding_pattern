...
class PriceFetcher:
    def __init__(self, scrip_code, symbol=None):
        self.scrip_code = scrip_code
        self.history = None
        
        # Prioritize symbol ticker over scrip code ticker
        ticker_options = []
        if symbol and not symbol.isdigit():
            ticker_options.append(f"{symbol}.BO")
        ...
        for ticker_opt in ticker_options:
            ...
            if self._fetch_history(ticker_opt):
                self.ticker_symbol = ticker_opt
                break
        else:
            self.ticker_symbol = f"{scrip_code}.BO"
